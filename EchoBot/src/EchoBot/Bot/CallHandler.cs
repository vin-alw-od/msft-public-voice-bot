...
namespace EchoBot.Bot
{
    ...
    public class CallHandler : HeartbeatHandler
    {
        ...
        {
            GraphLogger.Info($"Call status updated from {e.OldResource.State} to {e.NewResource.State} - {e.NewResource.ResultInfo?.Message}");
                ...
                if (_llmSpeechService != null)
                {
                    _ = Task.Run(async () =>
                    {
                        try
                        {
                            ...
                            GraphLogger.Info($"LLM speech service StartAsync completed for call: {this.Call.Id}");
                        }
                        catch (Exception ex)
                        {
                            GraphLogger.Error($"Failed to start LLM speech service: {ex.Message} | {ex.StackTrace}");
                        }
                    });
                }
                else
                {
                    GraphLogger.Error("LLM Speech Service is null - bot will be silent");
                }
            }
